...
- **点击即播**：解码在添加时后台完成（音频原样解码，不做裁剪），输出设备在启动时预热，按下到推流约 0.1~0.8ms（实测）；再点一次是"从头重播"，同样只要 ~0.6ms。
- **再次点击**：停止并从头重播。
- **右键菜单**：
  - 「无」（列表最上方）：不播放音效（按下动画照常）
  - 音效列表（点选切换并播放）
  - 添加音效…（多选文件）
  - 移除当前音效
  - 音量（滑杆）
...
- **记忆**：音效列表、当前音效（重启后自动恢复，选择「无」也会记住；配置里存为 `lastPlayed`）、音量、窗口位置、置顶状态存于 exe 旁 `config.json`，便携可带走。
...
音效文件自带的开头/结尾静音会原样播出：播放链路不修改音频数据（不裁剪、不增益、不重编码）。
...
src/SoundLibrary.*        音效列表：QAudioDecoder 预解码成内存 PCM（原样，不裁剪）、config.json 持久化
